...
const AccountDropdown = ({
  ...
  return (
    <div
      className={`absolute md:top-16 right-0 px-5 py-4 w-fit md:w-48 duration-700 bg-primary text-white ${
        showAccountDropdown
    ...
    >
      ...
      <div className="flex flex-col space-y-5">
        <Link
          className="flex items-center py-2 px-3 bg-white rounded text-primary text-center"
          href={"/dashboard/profile"}
        >
          <MdManageAccounts size={25} />
          <span className="mx-2 text-primary font-medium uppercase">
            Account
          </span>
        </Link>

        <button
          className="items-center flex px-3 py-2 bg-white text-primary rounded text-center"
          onClick={logout}
        >
          <CiLogout size={25} />
          <span className="mx-2 text-primary font-medium uppercase">
